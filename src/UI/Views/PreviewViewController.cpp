...
void PreviewViewController::DidActivate(bool firstActivation,
  ...
  if (firstActivation && grabber == nullptr) {
	...
	StartCoroutine(
	    custom_types::Helpers::CoroutineHelper::New(FinalizePanels()));
  }

  StartCoroutine(custom_types::Helpers::CoroutineHelper::New(
      MultiplierPreviewCoroutine()));
  grabber->get_transform()->set_position(DEFAULT_GRABBER_POS);
  ...
  if (firstActivation) {
	...
	auto headerText =
	    BeatSaberUI::CreateText(vertLayout->get_transform(), "Preview");
...
void PreviewViewController::DidDeactivate(bool removedFromHierarchy,
...
void PreviewViewController::UpdatePanelVisibility(int idx) {
  ...
  switch (idx) {
	case 0:
	  objectGrabber->multiplierPanel->get_transform()->set_localPosition(
	      VOID_VECT3);
	...
	case 4:
	  ...
	  objectGrabber->comboPanel->get_transform()->set_localPosition(
	      Vector3::get_zero());
	  break;
	case 5:
...
void PreviewViewController::UpdateEnergyBar() {
  ...
  if (fillAmount == 1.0F && config.rainbowFull) {
	energyBar->set_color(Color::HSVToRGB(
	    PreviewUtils::pingPong(Time::get_time() * 0.5F, 1.0F), 1.0F, 1.0F));
	return;
  }
  ...
  if (fillAmount > 0.5F && fillAmount < 1) {
	energyBar->set_color(Color::Lerp(config.midColor, config.highColor,
	                                 (fillAmount - 0.5F) * 2));
  }

  if (fillAmount < 0.5F) {
	energyBar->set_color(
	    Color::Lerp(config.lowColor, config.midColor, fillAmount * 2));
  }
}
...
void PreviewViewController::UpdateProgressBar() {
  ...
  progressPanelImages[2]->get_transform()->set_localPosition(
      Vector3(modifiedFill, 0, 0));
  progressPanelImages[0]->get_rectTransform()->set_anchorMax(
      Vector2(progressFillAmount, 1));

  if (config.enableGradient) {
	progressPanelImages[0]->set_color(
	    Color::Lerp(config.startColor, config.endColor, progressFillAmount));
  } else {
...
custom_types::Helpers::Coroutine PreviewViewController::FinalizePanels() {
  std::function<bool()> func = [this]() { return objectGrabber->isCompleted; };
  ...
  multiplierText =
      multiplierPanel->GetComponentsInChildren<CurvedTextMeshPro*>().Last();
  multiplierCircles =
      multiplierPanel->get_transform()->GetComponentsInChildren<Image*>();
  ...
  energyBar = objectGrabber->energyPanel->get_transform()
                  ->Find("EnergyBarWrapper/EnergyBar")
                  ->GetComponent<Image*>();

  comboLines =
      comboPanel->get_transform()->GetComponentsInChildren<ImageView*>();
  numText = comboPanel->get_transform()
                ->Find("ComboCanvas/NumText")
                ->GetComponent<CurvedTextMeshPro*>();
  comboText = comboPanel->get_transform()
                  ->Find("ComboText")
                  ->GetComponent<CurvedTextMeshPro*>();

  progressPanelImages =
      progressPanel->get_transform()->GetComponentsInChildren<Image*>();
  auto texts = progressPanel->get_transform()
                   ->GetComponentsInChildren<CurvedTextMeshPro*>();
  texts[2]->set_text("0");
  texts[3]->set_text("01");

  auto immediateRankTransform =
      objectGrabber->immediateRankPanel->get_transform();
  scoreText = immediateRankTransform->Find("ScoreText")
                  ->GetComponent<CurvedTextMeshPro*>();
  percentageText = immediateRankTransform->Find("RelativeScoreText")
                       ->GetComponent<CurvedTextMeshPro*>();
  rankText = immediateRankTransform->Find("ImmediateRankText")
                 ->GetComponent<CurvedTextMeshPro*>();
...
PreviewViewController::MultiplierPreviewCoroutine() {
  while (true) {
	std::function<bool()> func = [this]() { return previewToggleIsReady; };

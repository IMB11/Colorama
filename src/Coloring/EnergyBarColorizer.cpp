...
void EnergyBarColorizer::Update() {
  ...
  if(config.rainbowFull && (this->energyPanel->gameEnergyCounter->get_instaFail() || (energy == 1.0F && counter->get_energyType() == GameplayModifiers::EnergyType::Bar))) {
    mainImage->set_color(Color::HSVToRGB(EnergyBarUtils::pingPong(Time::get_time() * 0.5F, 1.0F), 1.0F, 1.0F));
  }
}
...
custom_types::Helpers::Coroutine EnergyBarColorizer::PrepareColors() {
  ...
  System::Func_1<bool>* predicate = custom_types::MakeDelegate<System::Func_1<bool>*>(predicate_func);
  co_yield reinterpret_cast<System::Collections::IEnumerator*>(WaitUntil::New_ctor(predicate));

  EnergyBarConfiguration config = getColoramaConfig().energyBarConfiguration.GetValue();
  IGameEnergyCounter* counter = this->energyPanel->gameEnergyCounter;

  if(counter->get_energyType() == GameplayModifiers::EnergyType::Battery) {
    ListW<UnityEngine::UI::Image*> segments = this->energyPanel->batteryLifeSegments;
  ...
  std::function<void(float)> rawFunc = std::bind(&EnergyBarColorizer::HandleEnergyChange, this, std::placeholders::_1);
  counter->add_gameEnergyDidChangeEvent(custom_types::MakeDelegate<System::Action_1<float>*>(rawFunc));
...
MAKE_HOOK_MATCH(GameEnergyUIPanel_InitHook, &GameEnergyUIPanel::Init, void, GameEnergyUIPanel* self) {
